...
NAME = "plotpilot_style_manufacturing"
MODULE = "style_manufacturing"
...
def _metadata() -> dict[str, bytes]:
    return {f"{DIST_INFO}/METADATA": ("Metadata-Version: 2.3\nName: plotpilot-style-manufacturing\nVersion: 0.1.0\nSummary: Immutable Style manufacture qualification and Data packaging Code Plugin\nRequires-Python: >=3.12,<3.13\n\n").encode(),
            f"{DIST_INFO}/WHEEL": b"Wheel-Version: 1.0\nGenerator: nap00-deterministic-wheel-builder/1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n",
            f"{DIST_INFO}/top_level.txt": b"style_manufacturing\n"}
...
def rebuild_package() -> dict[str,object]:
    ...
    identity={"schema":"style-manufacturing-package-identity/v1","plugin_id":PLUGIN_ID,"version":VERSION,"package_hash":digest.package_hash,"release_id":digest.release_id}
    ...
    for path in root.rglob("*"):
        if path.is_file() and "__pycache__" not in path.parts and path.suffix!=".pyc" and path.name not in {".gitattributes","expected.json","files.sha256"}: physical.append(path.relative_to(root).as_posix())
    indexes=list(INDEX_BY_CAPABILITY.values())
    projection=capability_projection()
    expected={"schema":"style-manufacturing-package-expected/v1","plugin_id":PLUGIN_ID,"version":VERSION,
        ...
        "wheel_path":wheel.relative_to(root).as_posix(),"wheel_import":MODULE,"plugin_path":"plugin.json","identity_path":f"{MODULE}/identity.json"}
    _write_json(root/"expected.json",expected)
    from style_manufacturing.package_identity import calculate_identity
    verified = calculate_identity(root)
    if verified.package_hash != digest.package_hash or verified.release_id != digest.release_id:
        raise ValueError("rebuilt style-manufacturing package identity did not verify")
    return expected
