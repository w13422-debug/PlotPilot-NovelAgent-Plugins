"""Package identity verification for the generated donor-analysis bundle."""
...
MODULE_NAME = "donor_analysis"
IDENTITY_SCHEMA = "donor-analysis-package-identity/v1"
...
def _read_identity(root: Path | None = None) -> dict[str, Any]:
    ...
    except Exception as exc: raise RuntimeError("donor-analysis package identity is unavailable") from exc
    if not isinstance(value, dict) or set(value) != set(_FIELDS): raise RuntimeError("donor-analysis package identity schema is invalid")
    if value["schema"] != IDENTITY_SCHEMA or value["plugin_id"] != PLUGIN_ID or value["version"] != VERSION: raise RuntimeError("donor-analysis package identity owner/version is invalid")
    for field in ("package_hash", "release_id"):
        if (not isinstance(value[field], str) or re.fullmatch(r"[0-9a-f]{64}", value[field]) is None
                or value[field] == "0" * 64):
            raise RuntimeError("donor-analysis package identity hash is invalid")
    return value
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"donor-analysis {label} is unavailable or invalid") from exc
    if not isinstance(value, dict): raise RuntimeError(f"donor-analysis {label} is invalid")
    return value

def _safe_file(root: Path, relative: str, label: str) -> Path:
    if not isinstance(relative, str) or not relative or "\\" in relative:
        raise RuntimeError(f"donor-analysis {label} path is invalid")
    path = root / relative
    try: path.resolve().relative_to(root.resolve())
    except ValueError as exc: raise RuntimeError(f"donor-analysis {label} escapes package") from exc
    if not path.is_file(): raise RuntimeError(f"donor-analysis {label} is missing: {relative}")
    return path
...
def _verify_descriptor_semantics(relative: str, descriptor: dict[str, Any], provider: dict[str, str]) -> None:
    ...
    if spec is None or set(descriptor) != set(_DESCRIPTOR_FIELDS):
        raise RuntimeError("donor-analysis descriptor path/schema is not frozen")
    expected = spec.descriptor(provider["release_id"])
    for field, expected_value in expected.items():
        if descriptor.get(field) != expected_value:
            raise RuntimeError(
                f"donor-analysis descriptor {field} does not match canonical runtime semantics"
            )
...
def _verify_expected(root: Path, identity: dict[str, Any], manifest: bytes, digest: Any) -> None:
    ...
    if set(expected) != set(_EXPECTED_FIELDS) or expected.get("schema") != "donor-analysis-package-expected/v1":
        raise RuntimeError("donor-analysis expected identity schema is invalid")
    if expected.get("plugin_id") != PLUGIN_ID or expected.get("version") != VERSION:
        raise RuntimeError("donor-analysis expected identity owner/version is invalid")
    if (expected.get("package_hash") != digest.package_hash
        ...
        raise RuntimeError("donor-analysis expected identity does not match SDK digest")
    ...
    if (expected.get("capability_projection") != projection
        ...
        raise RuntimeError("donor-analysis expected capability projection is not canonical")
    if _read_json(root / "plugin.json", "plugin manifest") != plugin_manifest():
        raise RuntimeError("donor-analysis plugin manifest differs from capability specification")
    if _read_json(root / "ui" / "metadata-only.json", "UI metadata") != ui_metadata():
        raise RuntimeError("donor-analysis UI projection differs from capability specification")
    ...
    if (not isinstance(declared_files, list) or declared_files != _physical_package_files(root)
        ...
        raise RuntimeError("donor-analysis package inventory is not exact")
    ...
    if (descriptor_paths != list(DESCRIPTOR_PATHS)
        ...
        raise RuntimeError("donor-analysis descriptor identity index is invalid")
    ...
    for relative in descriptor_paths:
        ...
        if (_sha256(path) != descriptor_hashes[relative]
            ...
            raise RuntimeError("donor-analysis descriptor identity does not match expected identity")
    ...
    if (not isinstance(index_paths, list) or not index_paths
        ...
        raise RuntimeError("donor-analysis schema identity index is invalid")
    ...
    if observed_artifacts != artifact_hashes:
        raise RuntimeError("donor-analysis schema artifacts do not match expected identity")
    for relative in index_paths:
        ...
        if _sha256(path) != index_hashes[relative] or index.get("schemas") != artifacts_by_index[relative]:
            raise RuntimeError("donor-analysis schema index does not match expected identity")
        for item in index.get("schemas", []):
            if not isinstance(item, dict) or set(item) != {"schema_id", "path", "sha256"}:
                raise RuntimeError("donor-analysis schema artifact entry is invalid")
            ...
            if item["sha256"] != actual or artifact_hashes.get(artifact_relative) != actual:
                raise RuntimeError("donor-analysis schema artifact hash is invalid")

def _manifest(root: Path) -> tuple[tuple[str, ...], bytes]:
    raw = (root / "files.sha256").read_bytes()
    if raw.startswith(b"\xef\xbb\xbf") or b"\r" in raw or not raw.endswith(b"\n") or raw.endswith(b"\n\n"): raise RuntimeError("donor-analysis manifest must be UTF-8 LF with one final newline")
    names: list[str] = []
    for line in raw.decode("utf-8").splitlines(keepends=True):
        match = _LINE.fullmatch(line)
        if match is None: raise RuntimeError("donor-analysis manifest line is malformed")
        name = match.group(1)
        if name in names or name == "files.sha256": raise RuntimeError("donor-analysis manifest has duplicate/self entry")
        names.append(name)
    if not names: raise RuntimeError("donor-analysis manifest is empty")
...
def canonical_payload_files(root: Path | None = None) -> dict[str, bytes]:
    ...
    for name in names:
        ...
        except ValueError as exc: raise RuntimeError("donor-analysis manifest escapes package") from exc
        if not path.is_file(): raise RuntimeError("donor-analysis manifest input is missing: " + name)
...
def calculate_identity(root: Path | None = None):
    ...
    if tuple(files) != names: raise RuntimeError("donor-analysis manifest order mismatch")
    digest = digest_package(files, PLUGIN_ID, VERSION)
    if digest.files_sha256 != manifest: raise RuntimeError("donor-analysis files.sha256 does not match SDK digest")
    if value["package_hash"] != digest.package_hash or value["release_id"] != digest.release_id: raise RuntimeError("donor-analysis identity does not match SDK digest")
