...
NAME = "plotpilot_donor_analysis"
MODULE = "donor_analysis"
...
def _metadata() -> dict[str, bytes]:
    return {f"{DIST_INFO}/METADATA": ("Metadata-Version: 2.3\nName: plotpilot-donor-analysis\nVersion: 0.1.0\nSummary: Exact Atom Claim and rereview Code Plugin\nRequires-Python: >=3.12,<3.13\n\n").encode(),
            f"{DIST_INFO}/WHEEL": b"Wheel-Version: 1.0\nGenerator: nap00-deterministic-wheel-builder/1.0\nRoot-Is-Purelib: true\nTag: py3-none-any\n",
            f"{DIST_INFO}/top_level.txt": b"donor_analysis\n"}
...
def rebuild_package() -> dict[str,object]:
    ...
    identity={"schema":"donor-analysis-package-identity/v1","plugin_id":PLUGIN_ID,"version":VERSION,"package_hash":digest.package_hash,"release_id":digest.release_id}
    ...
    expected={"schema":"donor-analysis-package-expected/v1","plugin_id":PLUGIN_ID,"version":VERSION,
    ...
    from donor_analysis.package_identity import calculate_identity
    verified = calculate_identity(root)
    if verified.package_hash != digest.package_hash or verified.release_id != digest.release_id:
        raise ValueError("rebuilt donor-analysis package identity did not verify")
    return expected
