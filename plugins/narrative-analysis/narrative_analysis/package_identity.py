"""Package identity verification for the generated narrative-analysis bundle."""
...
MODULE_NAME = "narrative_analysis"
IDENTITY_SCHEMA = "narrative-analysis-package-identity/v1"
...
def _read_identity(root: Path | None = None) -> dict[str, Any]:
    ...
    except Exception as exc: raise RuntimeError("narrative-analysis package identity is unavailable") from exc
    if not isinstance(value, dict) or set(value) != set(_FIELDS): raise RuntimeError("narrative-analysis package identity schema is invalid")
    if value["schema"] != IDENTITY_SCHEMA or value["plugin_id"] != PLUGIN_ID or value["version"] != VERSION: raise RuntimeError("narrative-analysis package identity owner/version is invalid")
    for field in ("package_hash", "release_id"):
        if (not isinstance(value[field], str) or re.fullmatch(r"[0-9a-f]{64}", value[field]) is None
                or value[field] == "0" * 64):
            raise RuntimeError("narrative-analysis package identity hash is invalid")
    return value
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"narrative-analysis {label} is unavailable or invalid") from exc
    if not isinstance(value, dict): raise RuntimeError(f"narrative-analysis {label} is invalid")
    return value

def _safe_file(root: Path, relative: str, label: str) -> Path:
    if not isinstance(relative, str) or not relative or "\\" in relative:
        raise RuntimeError(f"narrative-analysis {label} path is invalid")
    path = root / relative
    try: path.resolve().relative_to(root.resolve())
    except ValueError as exc: raise RuntimeError(f"narrative-analysis {label} escapes package") from exc
    if not path.is_file(): raise RuntimeError(f"narrative-analysis {label} is missing: {relative}")
    return path
...
def _physical_package_files(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        if (not path.is_file() or "__pycache__" in path.parts or path.suffix == ".pyc"
                ...
                or path.suffix == ".whl" or path.name in {".gitattributes", "expected.json"}):
            continue
...
def _verify_descriptor_semantics(relative: str, descriptor: dict[str, Any], provider: dict[str, str]) -> None:
    ...
    if spec is None or set(descriptor) != set(_DESCRIPTOR_FIELDS):
        raise RuntimeError("narrative-analysis descriptor path/schema is not frozen")
    expected = spec.descriptor(provider["release_id"])
    for field, expected_value in expected.items():
        if descriptor.get(field) != expected_value:
            raise RuntimeError(
                f"narrative-analysis descriptor {field} does not match canonical runtime semantics"
            )
...
def _verify_expected(root: Path, identity: dict[str, Any], manifest: bytes, digest: Any) -> None:
    ...
    if set(expected) != set(_EXPECTED_FIELDS) or expected.get("schema") != "narrative-analysis-package-expected/v1":
        raise RuntimeError("narrative-analysis expected identity schema is invalid")
    if expected.get("plugin_id") != PLUGIN_ID or expected.get("version") != VERSION:
        raise RuntimeError("narrative-analysis expected identity owner/version is invalid")
    if (expected.get("package_hash") != digest.package_hash
        ...
        raise RuntimeError("narrative-analysis expected identity does not match SDK digest")
    ...
    if (expected.get("capability_projection") != projection
        ...
        raise RuntimeError("narrative-analysis expected capability projection is not canonical")
    if _read_json(root / "plugin.json", "plugin manifest") != plugin_manifest():
        raise RuntimeError("narrative-analysis plugin manifest differs from capability specification")
    if _read_json(root / "ui" / "metadata-only.json", "UI metadata") != ui_metadata():
        raise RuntimeError("narrative-analysis UI projection differs from capability specification")
    ...
    if (not isinstance(declared_files, list) or declared_files != _physical_package_files(root)
        ...
        raise RuntimeError("narrative-analysis package inventory is not exact")
    ...
    if (descriptor_paths != list(DESCRIPTOR_PATHS)
        ...
        raise RuntimeError("narrative-analysis descriptor identity index is invalid")
    ...
    for relative in descriptor_paths:
        ...
        if (_sha256(path) != descriptor_hashes[relative]
            ...
            raise RuntimeError("narrative-analysis descriptor identity does not match expected identity")
    ...
    if (not isinstance(index_paths, list) or not index_paths
        ...
        raise RuntimeError("narrative-analysis schema identity index is invalid")
    ...
    if observed_artifacts != artifact_hashes:
        raise RuntimeError("narrative-analysis schema artifacts do not match expected identity")
    for relative in index_paths:
        path = _safe_file(root, relative, "schema index")
        index = _read_json(path, "schema index")
        capability = next((capability_id for capability_id, index_path in index_by_capability.items() if index_path == relative), None)
        if (capability is None
                or set(index) != {"schema", "plugin_id", "capability_id", "schemas"}
                or index.get("schema") != "provider-schema-index/v1"
                or index.get("plugin_id") != PLUGIN_ID
                or index.get("capability_id") != capability
                or _sha256(path) != index_hashes[relative]
                or index.get("schemas") != artifacts_by_index[relative]):
            raise RuntimeError("narrative-analysis schema index does not match expected identity")
        for item in index.get("schemas", []):
            if not isinstance(item, dict) or set(item) != {"schema_id", "path", "sha256"}:
                raise RuntimeError("narrative-analysis schema artifact entry is invalid")
            ...
            if item["sha256"] != actual or artifact_hashes.get(artifact_relative) != actual:
                raise RuntimeError("narrative-analysis schema artifact hash is invalid")

def _manifest(root: Path) -> tuple[tuple[str, ...], bytes]:
    raw = (root / "files.sha256").read_bytes()
    if raw.startswith(b"\xef\xbb\xbf") or b"\r" in raw or not raw.endswith(b"\n") or raw.endswith(b"\n\n"): raise RuntimeError("narrative-analysis manifest must be UTF-8 LF with one final newline")
    names: list[str] = []
    for line in raw.decode("utf-8").splitlines(keepends=True):
        match = _LINE.fullmatch(line)
        if match is None: raise RuntimeError("narrative-analysis manifest line is malformed")
        name = match.group(1)
        if name in names or name == "files.sha256": raise RuntimeError("narrative-analysis manifest has duplicate/self entry")
        names.append(name)
    if not names: raise RuntimeError("narrative-analysis manifest is empty")
...
def canonical_payload_files(root: Path | None = None) -> dict[str, bytes]:
    ...
    for name in names:
        ...
        except ValueError as exc: raise RuntimeError("narrative-analysis manifest escapes package") from exc
        if not path.is_file(): raise RuntimeError("narrative-analysis manifest input is missing: " + name)
...
def calculate_identity(root: Path | None = None):
    ...
    if tuple(files) != names: raise RuntimeError("narrative-analysis manifest order mismatch")
    digest = digest_package(files, PLUGIN_ID, VERSION)
    if digest.files_sha256 != manifest: raise RuntimeError("narrative-analysis files.sha256 does not match SDK digest")
    if value["package_hash"] != digest.package_hash or value["release_id"] != digest.release_id: raise RuntimeError("narrative-analysis identity does not match SDK digest")
...
def load_runtime_identity() -> dict[str, Any]:
    ...
    # An installed wheel is only executable when its identity sidecars are
    # present and replayable.  Returning a parseable identity while silently
    # skipping a missing manifest would allow an incomplete/tampered install
    # to advertise a forged release.
    if not manifest.is_file():
        raise RuntimeError("narrative-analysis files.sha256 manifest is unavailable")
    try:
        calculate_identity(package_root())
    except ModuleNotFoundError as exc:
        if not exc.name or not exc.name.startswith("plotpilot_plugin_sdk"):
            raise
        raise RuntimeError("narrative-analysis public SDK is unavailable") from exc
    return value
